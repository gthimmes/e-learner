...
| Global | **Per-organization branding** (name, logo, colour, tagline) applied across the app for members; **UI in English, Spanish and French** with a footer switcher (cookie + Accept-Language); **WCAG 2.2 AA** automated audit (axe) in the e2e suite, skip link, labelled controls; **installable PWA** whose service worker keeps opened lessons readable offline |
| Live | **Live sessions** per course or cohort with RSVP, join link, calendar invites (`.ics` emailed and downloadable), "live now" state; **recordings** attached to lessons afterwards; **office hours** — instructors open slots, learners book one with a topic, hosts get notified |
