...
const courses = await db.course.deleteMany({
  where: { OR: [{ title: { startsWith: "E2E Course" } }, { title: { startsWith: "Discover Course" } }, { title: { startsWith: "Paid Course" } }, { slug: { startsWith: "e2e-" } }, { slug: { startsWith: "disc-" } }, { slug: { startsWith: "paid-" } }] },
});
const paths = await db.learningPath.deleteMany({ where: { title: { startsWith: "Zebra Path" } } });
const users = await db.user.deleteMany({
  where: { OR: [{ email: { startsWith: "e2e-" } }, { email: { startsWith: "disc-" } }, { email: { startsWith: "buyer-" } }, { email: { startsWith: "dana.demo" } }, { email: { startsWith: "sso-" } }, { email: { startsWith: "reset-" } }, { email: { startsWith: "newbie" } }, { email: { startsWith: "coauthor-" } }] },
});
